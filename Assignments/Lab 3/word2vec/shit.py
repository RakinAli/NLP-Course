...
class Word2Vec(object):
    ...
    def create_unigram_dist(self):
        for unique_word in self.__unigram:
            self.__unigram[unique_word] = (
                self.__unigram[unique_word] / self.__total_words
            )
        ...
        for ep in range(self.__epochs):
            for i in tqdm(range(N)):
                ...
                self.gradient_descent(
                    focus_word_ind, pos_sample_words, negative_samples, words_seen
                )

    def gradient_descent(
        self, focus_word_ind, positive_samples, negative_samples, words_seen
    ):
        """
        ...
        focus_grads = self.grads_wrt_focusVec(
            focus_word_ind, positive_samples, negative_samples
        )
    ...
    def update_learning_rate(self, words_seen):
        ...
        if self.__lr < self.__init_lr * 0.0001:
            ...
            self.__lr = self.__init_lr * (
                1 - words_seen / (self.__epochs * len(self.__i2w) + 1)
            )
    ...
    def compute_pos_grads(self, focus_word_idx, pos_samples):
        ...
        gradients = np.outer(
            sigmoid_minus_one, focus_vec
        )  # Shape: (num_pos_samples, self.__H)

        # Create a dictionary mapping from indices to gradient vectors
        pos_samples_gradients_dict = {
            idx: gradients[i] for i, idx in enumerate(pos_indices)
        }
    ...
    def grads_wrt_focusVec(self, focus_word_ind, positive_samples, negative_samples):
        ...
        pos_gradients = pos_vectors * (
            self.sigmoid(pos_dot_products)[:, np.newaxis] - 1
        )
    ...
    def generate_neg_sample(self, focus_word_ind, positive_sample_indices):
        ...
        for pos_ind in positive_sample_indices:
            negative_samples.extend(
                self.negative_sampling(self.__nsample, focus_word_ind, pos_ind)
            )
        return negative_samples
